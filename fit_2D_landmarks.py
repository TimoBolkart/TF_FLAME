...
def fit_lmk2d(target_img, target_2d_lmks, template_fname, tf_model_fname, lmk_face_idx, lmk_b_coords, weights):
    ...
    with tf.Session() as session:
        ...
        lmks_proj_2d = tf_project_points(lmks_3d, tf_scale, trans)
        ...
        def on_step(target_img, target_lmks, opt_lmks, lmk_dist=0.0, shape_reg=0.0, exp_reg=0.0, neck_pose_reg=0.0, jaw_pose_reg=0.0, eyeballs_pose_reg=0.0):
        ...
        optimizer.minimize(session, fetches=[tf.constant(target_img), tf.constant(target_2d_lmks), lmks_proj_2d], loss_callback=on_step)
...
def run_2d_lmk_fitting():
    ...
    result_mesh = fit_lmk2d(target_img, lmk_2d, template_fname, tf_model_fname, lmk_face_idx, lmk_b_coords, weights)
